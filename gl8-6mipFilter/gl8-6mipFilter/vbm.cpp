...
bool VBObject::LoadFromVBM(const char * filename, int vertexIndex, int normalIndex, int texCoord0Index)
{
	...
	if (m_header.num_attribs != 0)
    {
        m_material = new VBM_MATERIAL[m_header.num_attribs];
        memcpy(m_material, raw_data + total_data_size, m_header.num_attribs * sizeof(VBM_MATERIAL));
        total_data_size += m_header.num_attribs * sizeof(VBM_MATERIAL);
        m_material_textures = new VBObject::material_texture[m_header.num_attribs];
        memset(m_material_textures, 0, m_header.num_attribs * sizeof(*m_material_textures));
    }

    if (m_header.num_attribs != 0)
    {
        m_chunks = new VBM_RENDER_CHUNK[m_header.num_attribs];
        memcpy(m_chunks, raw_data + total_data_size, m_header.num_attribs * sizeof(VBM_RENDER_CHUNK));
        total_data_size += m_header.num_attribs * sizeof(VBM_RENDER_CHUNK);
    }
